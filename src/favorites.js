/* AFFOFavorites: private UI state with explicit popup callbacks and live state getters. */
(function(root) {
'use strict';
...
function create(popup) {
    ...
    function getSavedFavorites() { return savedFavorites; }
    ...
    function generateDetailedFavoritePreview(config) {
        ...
        if (config.variableAxes && config.activeAxes) {
            const activeAxesEntries = Object.entries(config.variableAxes)
                .filter(([axis, value]) => {
                    const fontDef = popup.getEffectiveFontDefinition(config.fontName);
    ...
    function generateFontConfigName(position) {
        const config = popup.getCurrentUIConfig(position);
    ...
    function showSaveModal(position) {
        if (typeof popup.getCurrentPanelState === 'function' && popup.getCurrentPanelState(position).kind === 'sroulette') {
            return;
        }
    ...
    async function isActivePageSubstackForFavorites(origin) {
        ...
        try {
            if (typeof popup.getTargetTabForPopup !== 'function' || typeof AFFOMessaging === 'undefined') return false;
            const tab = await popup.getTargetTabForPopup();
    ...
    async function loadSrouletteFavoritesForPopup(position) {
        ...
        const origin = typeof popup.getActiveOrigin === 'function' ? await popup.getActiveOrigin() : null;
    ...
    async function showFavoritesPopup(position) {
        ...
        const popupElement = document.getElementById('favorites-popup');
        ...
        affoDebugLog('Favorites popup elements:', {popup: popupElement, listContainer, noFavorites, searchInput});
        ...
        if (filteredNames.length === 0 && filteredSrouletteEntries.length === 0) {
            ...
            filteredSrouletteEntries.forEach(entry => {
                ...
                item.addEventListener('click', async function() {
                    ...
                    try {
                        if (typeof popup.markPanelAsSroulette === 'function') {
                            popup.markPanelAsSroulette(position, pool);
                        }
                        if (position === 'body') {
                            await popup.updateBodyButtons();
                        } else if (popup.currentViewMode === 'third-man-in') {
                            await popup.updateAllThirdManInButtons(position);
                        }
                    } catch (error) {
                ...
                item.addEventListener('click', async function() {
                    ...
                    if (config) {
                        try {
                            ...
                            await popup.applyFontConfig(position, config);
                            ...
                            if (position === 'body') {
                                ...
                                await popup.updateAllThirdManInButtons();
                            } else if (popup.currentViewMode === 'faceoff') {
                                popup.saveExtensionState();
                            }
        ...
        if (names.length === 0) {
            ...
            filteredNames.forEach(name => {
                ...
                deleteBtn.addEventListener('click', function() {
                    popup.showCustomConfirm(`Are you sure you want to delete "${name}"?`).then(confirmed => {
                        if (confirmed) {
    ...
    function setupEventListeners() {
        ...
        saveModalSave.addEventListener('click', function() {
            ...
            if (!name) {
                popup.showCustomAlert('Please enter a name for this favorite');
                return;
            }
            ...
            hideSaveModal();
            popup.showCustomAlert(`Saved "${name}" to favorites!`);
        });
        ...
        for (const position of ['top', 'bottom', 'body', 'serif', 'sans', 'mono']) {
            setupSaveFavorite(position);
            setupLoadFavorite(position);
        }
    }
    ...
    return {
        ...
        saveFavoritesToStorage,
        hasInCollection,
        getOrderedFavoriteNames,
        generateFavoritePreview,
        generateDetailedFavoritePreview,
        generateFontConfigName,

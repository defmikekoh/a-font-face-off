...
- Android Selenium/geckodriver clears the selected Firefox package data when creating a session. The user has pre-approved that destructive automation only for Firefox Nightly (`org.mozilla.fenix`) on the Samsung Galaxy Note10 with ADB serial `RF8M81WSL1V`. Do not target other Firefox packages on that Note10 or Firefox on any other Android device without new explicit approval. `web-ext run -t firefox-android` uses live Fenix state and may modify extension state, but its `--adb-remove-old-artifacts` flag removes web-ext artifacts rather than Firefox app data.

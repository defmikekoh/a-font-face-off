...
- Domain settings are merged per origin using `domains.json` + `domains-meta.json`. Non-Substack Sroulette stores only the target (`body`, `serif`, or `sans`) and source pool in `affoApplyMap[origin].sroulette`; the random resolved font is intentionally not synced.

...
- `content.js`: TMI inline application also keeps a non-persisted `inlineConfigs[fontType].tmiProtection` object. Its `normalEntries`, `boldProps`, and `boldEntries` are derived once from the config's CSS properties and effective weight, then reused for initial application, added subtrees, and recovery. Replacing the inline config replaces these prepared values; element boldness is still read per application to handle reused page nodes.
- `content.js`: `inlineWorkQueue` holds `{ iterator, cfg, fontType, resolve, reject }` jobs and `pendingInlineWorkChunk` owns their next timer/event continuation. Each job must still own `inlineConfigs[fontType]` to advance. A config's temporary `application` promise coordinates Apply completion and anchor restoration. `inlineRecoveryPromise` coalesces recovery calls, while `inlineRecoveryFullRequested` preserves a full-recovery request arriving during a polling pass. `inlineCssValues` caches at most 256 CSSOM-serialized property/value pairs using a detached parser; none of this state is persisted or synced.

...
- `content.js`: `lazyGoogleSubsetObservers` maps font names to `{ check: Function, timeoutId: number|null }`. Each callback owns its remaining subset URLs. `sharedLazySubsetObserver`, `sharedLazySubsetTimer`, and `pendingLazySubsetRoots: Set<Node>` share mutation collection and a bounded changed-text scan across fonts. Removing the final font cancels the timer, disconnects the observer, and releases pending DOM references.
...
- `content.js`: TMI inline application keeps `inlineConfigs[fontType].tmiProtection`: `normalEntries`, `boldProps`, `boldEntries`, `normalRecords`, and `boldRecords`, derived once per config. A record is `{ prop, value, canonical, priority, customProp, attribute }`. The config's `comparison` references normal/bold records and adds `fontName`, an optional color record, and optional scale metadata `{ factor, scaledAttr, originalAttr, selector }`. Replacing a config replaces these prepared values; element boldness is still read per batch to handle reused page nodes.
- `content.js`: `inlineWorkQueue` holds `{ iterator, cfg, fontType, batch, resolve, reject }` jobs and `pendingInlineWorkChunk` owns their next task/event continuation. A lazily created `inlineWorkChannel` (`MessageChannel`) posts task continuations, alternating with animation frames on visible pages to avoid nested timer delays without starving rendering. Hidden pages use message tasks, including a visibility-change wakeup for a suspended frame. `inlineWorkChunkSequence` rejects an old posted message after the popup bridge has already advanced that chunk. Single-owner jobs must still own `inlineConfigs[fontType]`; composite jobs check each group's config independently. `pendingTmiProtectionBatch` holds `{ groups, headingRoots: Set<Element>, promise }` until reading begins. Groups supply `{ cfg, fontType, elements|roots, repairOnly }` and gain transient `selector`, `discoveredElements`, `targets`, `boldness`, `candidates`, `applied`, and `scaleVerification` fields. Batch-local config state deduplicates queued/checked elements; shared `scaleVerification` holds `{ checked: WeakSet<Element>, repairs: Set<Element>, full: boolean }` and is discarded after recovery. A config's temporary `application` promise coordinates Apply completion and anchor restoration. `inlineRecoveryPromise` coalesces recovery calls, while `inlineRecoveryFullRequested` preserves a full-recovery request arriving during a polling pass. `inlineCssValues` caches at most 256 CSSOM-serialized property/value pairs using a detached parser; none of this state is persisted or synced.
...
### Binary font messages

`affoFetch` binary replies use `{ ok: true, binary: true, data: "<base64>", encoding: "base64", cached: boolean }` on both engines. Chromium JSON messaging cannot carry ArrayBuffers. `AFFOMessaging` encodes at the background message boundary and decodes before creating a FontFace in content scripts. The background runtime and IndexedDB continue to use native ArrayBuffers; no cache schema changes are needed.

### MV3 session CSS tracking

In both Firefox and Chromium MV3 builds, `browser.storage.session` stores `affoSrouletteInsertedCss:<tabId>` objects, mapping configured TMI targets (`serif`, `sans`, `mono`) to the exact inserted CSS strings. This allows removal of both author- and user-origin styles after an event-page or service-worker restart. Updates are serialized per tab, and entries are removed when cleared or when the tab closes. This bookkeeping is not synced and expires with the browser session. The in-memory map is a cache of the session data.
...
| `affoGDriveFolderSuffix` | Optional Google Drive sync folder suffix | `"dev"` creates remote folder `A Font Face-off dev` |

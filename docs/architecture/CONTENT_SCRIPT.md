...
- **Drop cap preservation**: Elements matching semantic drop-cap hints in `style` (`var(--drop-cap)` or `initial-letter`), `class`, `data-drop-cap`, `data-dropcap`, or `data-testid` are excluded as containers. This preserves decorative initials such as Guardian spans using `var(--drop-cap)` while allowing the surrounding paragraph text to be replaced.

...
- **Direct-text requirement**: The TMI walker only marks elements with their own text nodes. Structural wrappers that only inherit text from descendants are skipped, which prevents ancestor font markers from leaking onto excluded descendants like headings.
- **Container exclusions (ancestor-aware)**: Elements inside `figcaption`, `button`, `.post-header`, Substack top-bar chrome (`.main-menu`, `[class*="topBar"]`), or `[role="dialog"]` are always excluded. When the current origin is in `affoIgnoreCommentsDomains`, AFFO also excludes `.comments-page` comment views. Body-mode CSS mirrors both the `.post-header` and domain-gated `.comments-page` exclusions with `:not(...)` selectors.

...
- **Shared exclusions**: heading subtrees (h1-h6 and descendants), code/pre/kbd, nav, form elements, ARIA roles (navigation, button, tab, toolbar, menu, alert, status, log), syntax highlighting classes, small-caps, metadata/bylines, widget/ad patterns, WhatFont compatibility
...
- **Container exclusions (ancestor-aware)**: Elements inside `figcaption`, `button`, `.post-header`, Substack top-bar chrome (`.main-menu`, `[class*="topBar"]`), interactive modal roots (`[role="dialog"]`, `[role="alertdialog"]`, `[aria-modal="true"]`), or any fixed-position subtree are always excluded. Fixed-position ancestry is determined from computed styles rather than class names, catching site overlays and banners that omit dialog semantics; results are cached for each full or mutation-scoped walker pass. TMI's TreeWalker rejects semantic modal roots, skipping their whole subtree, and shared mutation observers ignore additions inside those subtrees so React-heavy create/edit dialogs do not trigger wasted re-walks. On Substack pages, styled `p.button-wrapper > a.button.primary > span` button links are also skipped so link-buttons and real buttons are treated consistently. AFFO also excludes likely article decks/standfirsts in `article header` when the matching `p`/`div` carries deck-style hints such as `summary`, `subtitle`, `dek`, `deck`, `standfirst`, `subheadline`, or `excerpt`. The walker requires those hints on the candidate `p`/`div` itself (or an ancestor up to the header) and rejects broad `div` wrappers that contain multiple nested block children. When the current origin is in `affoIgnoreCommentsDomains`, AFFO also excludes `.comments-page` comment views. Body-mode CSS mirrors the `.post-header`, article-deck, and domain-gated `.comments-page` exclusions with `:not(...)` selectors.
- **Drop cap preservation**: Elements matching semantic drop-cap hints in `style` (`var(--drop-cap)` or `initial-letter`), exact `class` tokens, `data-drop-cap`, `data-dropcap`, or `data-testid` are excluded as containers. Exact class-token matching avoids rejecting article wrappers with generated classes such as `dropCap-hash`. This preserves decorative initials such as Guardian spans using `var(--drop-cap)` while allowing the surrounding paragraph text to be replaced.
...
- **Percent font-size scaling**: `fontSizeScale` is applied by content.js, not as cascading `%` CSS. The content script records each matched element's original computed `font-size`, writes the scaled px value inline, and restores the prior inline `font-size` when scaling is removed. Body Contact scale targets also include article/content headings (`h1`-`h6`), likely article deck text, and x.com tweet author clusters (`[data-testid="User-Name"]`) so site hierarchy scales together, while family/weight/style replacement still excludes headings.
...
### Selector Routing
- **`BODY_EXCLUDE`** — Effective chain assembled by `getBodyExcludeSelector()`: `:not(h1):not(h2)...:not(.no-affo):not([data-affo-guard]):not([data-affo-guard] *)` plus structural exclusions like `.post-header`, `.comments-page`, article-deck selectors inside `article header`, and drop-cap selectors.

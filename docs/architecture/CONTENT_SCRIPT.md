...
- **Container exclusions (ancestor-aware)**: Elements inside `figcaption`, `button`, `.post-header`, or `[role="dialog"]` are always excluded. When the current origin is in `affoIgnoreCommentsDomains`, AFFO also excludes `.comments-page` comment views. Body-mode CSS mirrors both the `.post-header` and domain-gated `.comments-page` exclusions with `:not(...)` selectors.

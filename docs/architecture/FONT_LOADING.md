...
TMI Apply reuses the page's completed classification when switching fonts. A required scan can also progress through the popup's polling bridge, which advances one pending chunk per poll without waiting on source-tab timers. Previously every Apply invalidated classification and started another timer-driven scan; on Firefox Android a hidden source tab could suspend those timers, leaving Apply waiting until the walker polling limit even when both fonts were already loaded.
...
1. **Early background warming from `left-toolbar.js`** (`document_start`): Reads `affoApplyMap` from storage and sends `affoWarmFontFace` only for the saved mode's remote families: `body` for Body Contact or the present `serif`/`sans`/`mono` positions for TMI. The stored domain shape selects the mode in normal data; mixed legacy/synced entries use `affoCurrentView`, then legacy `affoCurrentMode`, and conservatively warm all present positions only when neither identifies an applied mode. The background resolves/fetches Google CSS and warms the configured current-weight Latin WOFF2 in IndexedDB. The warming step adds no page-level stylesheet. Apply Early domains also start the main content runtime once head/body exist; their font loading and CSS application can therefore begin while the page is still parsing.
...
3. **Explicit face readiness in `content.js`**: Google CSS and selected font binaries are fetched through the background, then registered as `FontFace` objects on every domain. This avoids both load-bearing page `<link>` elements and site CSP restrictions on page-level font URLs. The reapply path then calls `document.fonts.load()` for the configured face plus the supplemental bold face. Custom paths use either their existing CSS rules or the same binary `FontFace` mechanism before the readiness check. It does not wait on `document.fonts.ready`, which could be delayed by unrelated site fonts.
4. **Bounded stable visible swap**: AFFO keeps the page's current font styling while the configured face becomes ready, captures a readable element near the top of the viewport, applies the complete CSS change once, and restores that element's viewport offset over the next layout frames. Readiness is capped at five seconds for persisted page application; on error or timeout AFFO applies the configured family CSS and lets the browser use its fallback until the face becomes available. TMI CSS does not wait for the element walker to finish; it begins matching as markers are added. Popup-triggered applies use the same content-script preparation/anchor bridge before injecting user-origin CSS.

Result: fetching still starts at `document_start`, but font work cannot keep the page load pending indefinitely or suppress a saved configuration. The normal path retains the stable swap, while the bounded fail-open path prefers usable page styling over an unbounded wait.
...
Google fonts no longer rely on page-level Google Fonts `<link>` injection. On every domain, `content.js` asks `background-font-runtime.js` to fetch the Google Fonts CSS, select matching WOFF2 subsets, and fetch font binaries via `affoFetch`.

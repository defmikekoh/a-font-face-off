...
On Note10 Firefox Nightly 155.0a1, a fresh geckodriver session loaded the requested
article and executed AFFO while Firefox's onboarding screen remained in front.
After onboarding, Firefox Home still covered the article. The page had a
zero-width viewport, yet DOM queries, font loading, and toolbar rectangle checks
succeeded. Native WebDriver clicks failed or did not open Quick Pick. These were
not valid visible-page measurements.
...
When the native UI shows onboarding, dismiss its observed Continue/Not now
controls, then the add-on-added dialog if present. If Firefox Home remains,
select the already-loaded article from its title in Jump back in or the tab
switcher. Locate controls from current UI nodes rather than fixed coordinates.
Recheck the target URL, viewport, and screenshot afterward. The inspector has
onboarding helpers for bookmark restoration after teardown; do not assume this
means onboarding was dismissed before its page inspection.

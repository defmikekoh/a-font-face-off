...
- ADB transport health is separate from device awake state, foreground UI, and internet health. Keep Note10 awake throughout testing using the [parent skill procedure](../SKILL.md#keep-note10-awake-throughout-testing), restoring the original setting afterward. Wake/dismiss a non-secure keyguard within existing authorization, then inspect the current UI before navigating. Refresh the UI tree after a delay before using tap coordinates. Exclude any sleep-interrupted run from timing comparisons.
